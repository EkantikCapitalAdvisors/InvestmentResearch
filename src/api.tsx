...
apiRoutes.post('/auth/verify-passcode', async (c) => {
  const secret = c.env.RESEARCH_PASSCODE
...
apiRoutes.get('/auth/passcode-required', async (c) => {
  const secret = c.env.RESEARCH_PASSCODE
...
apiRoutes.get('/observations/files', async (c) => {
...
apiRoutes.post('/observations/files', async (c) => {
...
// Download a file
apiRoutes.get('/observations/files/:id/download', async (c) => {
  const id = c.req.param('id')
...
apiRoutes.get('/observations/files/:id/preview', async (c) => {
  const id = c.req.param('id')
  const file = await c.env.DB.prepare('SELECT * FROM observation_files WHERE id = ?').bind(id).first() as any
  if (!file) return c.json({ error: 'File not found' }, 404)
...
// Delete a file
apiRoutes.delete('/observations/files/:id', async (c) => {
  const id = c.req.param('id')
